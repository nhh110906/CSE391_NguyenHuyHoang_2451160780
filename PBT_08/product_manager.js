...
// 1. Lọc sản phẩm còn hàng (filter)
function getInStock(products) {
    return products.filter(product => product.stock > 0);
}

// 2. Lọc theo category và khoảng giá (filter)
...
// 3. Sắp xếp theo giá tăng/giảm (sort)
...
// 4. Tìm sản phẩm rẻ nhất mỗi category (reduce + find)
function cheapestByCategory(products) {
    ...
    return categories.reduce((result, category) => {
        ...
        const minPrice = Math.min(
            ...productsInCategory.map(product => product.price)
        );

        result[category] = productsInCategory.find(
...
// 5. Tính tổng giá trị kho (reduce)
...
// 6. Tạo mảng { name, formattedPrice } (map)
...
// 7. Tính rating trung bình (reduce)
...
// 8. Tìm sản phẩm theo keyword (filter)
...
console.log("\n=== FIND PRODUCT ID = 5 ===");
console.log(findById(products, 5));